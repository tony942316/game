 ...
 */
public class ShopFTWz
{
    public static void main(String[]args)
    {
        Player player = Setup.getPlayer();
        String[] items = {"*Sword*: 150 coins", "*Health* Potion: 250 coins", "*Heart* Amulet: 300 coins", "Rabbit's *Foot*: " + player.getHCost() + " coins"};
        if(player.getSword())
        {
            items[0] = "Sharpen *Sword*: 200 coins";
        }
        if(player.getSwordSTR() >= 3)
        {
        ...
        {
            items[3] = "Rabbit's *Foot*: " + player.getHCost() + " coins";
            ...
            if(action.equalsIgnoreCase("sword"))
            {
                ...
                else if(player.getCoins() >= 200 && player.getSword())
                {
                    ...
                    player.changeSwordSTR();
                }
                else if(player.getCoins() >= 150 && !player.getSword())
                {
            ...
            else if(action.equalsIgnoreCase("Foot"))
            {
                ...
                else if(player.getCoins() >= player.getHCost())
                {
                    player.changeCoins(-player.getHCost());
